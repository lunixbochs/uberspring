...
if len(sys.argv) < 2 or not sys.argv[1]:
	print 'Cannot continue: no demo file specified.'
	print 'Sleeping for 60 seconds in case console autoexits'
	time.sleep(60)
	sys.exit(1)
...
def splitpath(path):
	...
	return path.rsplit(slash,1)[0]

def testpath():
	try:
		f = open('write.test', 'w')
		f.close()
		os.remove('write.test')
		return True
	except:
		return False

if not testpath():
	basepath = splitpath(sys.argv[0])
	os.chdir(basepath)
	if not testpath():
		basepath = splitpath(sys.argv[1])
		os.chdir(basepath)
		if not testpath():
			print 'Cannot continue: no writable path.'
			print 'Sleeping for 60 seconds in case console autoexits'
			time.sleep(60)
			sys.exit(1)
...
def parsePacket(packet):
	...
	if cmd == 1:
		...
		write(locals(), 'cmd', 'countdown')
	elif cmd == 5:
		cmd = 'setplayernum'
		playerNum = ord(data)
		write(locals(), 'cmd', 'playerNum')
	elif cmd == 6:
		cmd = 'setplayername'
		...
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'msgsize', 'playerNum', 'playerName', 'unitIDCount', 'unitIDs', 'commands')
	elif cmd == 16:
		cmd = 'aishare'
		...
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'sourceTeam', 'destTeam', 'metal', 'energy', 'unitIDCount', 'unitIDs')
	elif cmd == 17:
		cmd = 'memdump'
		...
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'userSpeed')
	elif cmd == 20:
		cmd = 'internal_speed'
		internalSpeed = struct.unpack('<f', data)[0]
		write(locals(), 'cmd', 'internalSpeed')
	elif cmd == 21:
		cmd = 'cpu_usage'
		...
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'status', 'heading', 'pitch')
	elif cmd == 25:
		cmd = 'attemptconnect'
		size = struct.unpack('<H', data[0])[0]
		name, password, version = data[1:].split('\0',2)
		version = version.strip('\0')
		write(locals(), 'cmd', 'size', 'name', 'password', 'version')
	elif cmd == 26:
		cmd = 'share'
		playerNum, shareTeam, shareUnits, shareMetal, shareEnergy = struct.unpack('<3Bff', data)
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'shareTeam', 'shareUnits', 'shareMetal', 'shareEnergy')
	elif cmd == 27:
		cmd = 'setshare'
		playerNum, team, metalShareFraction, energyShareFraction = struct.unpack('<BBff', data)
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'team', 'metalShareFraction', 'energyShareFraction')
	elif cmd == 28:
		cmd = 'sendplayerstat'
		write(locals(), 'cmd')
	elif cmd == 29:
		...
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'frameNum', 'checksum')
	elif cmd == 35:
		cmd = 'systemmsg'
		size, playerNum = struct.unpack('<BB', data[:2])
		message = data[2:]
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'size', 'playerNum', 'playerName', 'message')
	elif cmd == 36:
		cmd = 'startpos'
		...
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'bIntended', 'readableIntended')
	elif cmd == 50:
		cmd = 'luamsg'
		size, playerNum, script, mode = struct.unpack('<HBHB', data[:6])
		msg = data[6:]
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'size', 'playerNum', 'playerName', 'script', 'mode', 'msg')
	elif cmd == 51:
		cmd = 'team'
		...
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'action', 'param')
	elif cmd == 52:
		cmd = 'gamedata'
		...
		if len(data) == 12: # 0.80 or later
			...
			write(locals(), 'cmd', 'size', 'compressedSize', 'setupText', 'mapChecksum', 'modChecksum', 'randomSeed')
		elif False:
			print __import__('binascii').hexlify(data)
		...
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'playerNum', 'playerName', 'otherAllyTeam', 'allianceState', 'readableAllianceState')
	elif cmd == 54:
		cmd = 'ccommand'
		size, playerNum = struct.unpack('<Hi', data[:6])
		command, extra = data[6:].split('\0',1)
		playerName = players[playerNum] or ''
		write(locals(), 'cmd', 'size', 'playerNum', 'playerName', 'command', 'extra')
	elif cmd == 60:
		cmd = 'teamstat'
		data = 'unparsed'
		write(locals(), 'cmd', 'data')
...
while packet:
	...
	try:
		...
		print error
		print '-'*20
		er = open('errors.txt', 'w')
		er.write('\n'+error+'\n')
		er.close()
		
f.close()

print 'Parsed demo was written to output.txt, any errors were written to errors.txt'
print
print 'Sleeping for 60 seconds in case console autoexits.'
time.sleep(60)